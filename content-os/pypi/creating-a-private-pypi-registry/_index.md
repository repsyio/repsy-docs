...
When the first user of an instance is created, Repsy creates a private PyPI repository named `pypi` (and one private repository of every other type). If you only need one PyPI repository, you can use it right away.
...
A PyPI repository has two addresses, both made of the repository address of your instance and the repository name:

| Use | Address |
| --- | --- |
| Upload (`twine`) | `{{% repo-url %}}/<repo-name>` |
| Install (`pip`), as the index URL | `{{% repo-url %}}/<repo-name>/simple` |
...
Open the repository in the web UI and click **Configure** to see a `.pypirc` snippet that already contains the upload URL and the name of the repository.

### Get your credentials

`twine` and `pip` authenticate with HTTP Basic credentials, and there are two choices:
