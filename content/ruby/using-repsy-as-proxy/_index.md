...
A Ruby proxy repository acts as an intermediary between your gem clients and external sources. When you request a gem that does not exist locally, the proxy automatically fetches it from the configured upstream sources and caches it for future use.
...
1. **Client Request**: gem or Bundler requests a gem from your `Repsy` repository.
...
- Point your `Gemfile` or `gem install --source` to your Repsy repository URL — proxy resolution happens transparently on the server side.
