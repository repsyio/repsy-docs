...
3. **The size.** The zip file can be as large as `GO_MAX_MODULE_ZIP_SIZE` allows, 500 MB by default. A client that announces a larger file is refused before the body is read, and so is an upload that grows past the limit while Repsy reads it, such as a chunked one. What the zip unpacks to has a limit of its own, 500 MiB in total and 100,000 files, and so has its `go.mod`, 16 MiB. An administrator changes the first limit with the variable, see the [Configuration Reference](../../installation/configuration-reference/#upload-size-limits).
